...
export const OrdersApi = {
  addToCart: (token: string, data: string): Promise<CartDataType> => {
  ...
  removeOne: (token: string, data: string): Promise<CartDataType> => {
    return axios.put('/api/cart', {id: data}, {headers: {authorization: token}})
  },
  removeAll: (token: string, data: string): Promise<CartDataType> => {
  ...
  cancelOrder: (token: string, data: string): Promise<OrdersDataType> => {
